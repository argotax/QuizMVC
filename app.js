  ...
  .io.on('connection', function(socket) {
    ...
    socket.on('friend-request', function(friendRequest) {
      models
        ...
        .findOne({
          where: {
            user_login: friendRequest.friend
          },
          attributes: ['user_id']
        }).then(
          user => {
    ...
    socket.on('confirm-request', function(confirmRequest) {
      models.broquiz_friend.update({
        ...
        .then(result =>
          socket.emit('friend-confirmed', {
            data: confirmRequest.friend
          })
        )
        .catch(err =>
    ...
    socket.on('player-answer', function(answerResponse) {
        ...
        models.broquiz_round.findAll({
          ...
          round => {
            if (((round[round.length - 1].round_q1_p1 == undefined) && (round.length % 2 == 1)) || ((round[round.length - 1].round_q1_p2 != undefined) && (round.length % 2 == 0))) {
              models.broquiz_round.update({
                round_p1_score: score,
                round_q1_p1: answerResponse.answer[0],
                round_q2_p1: answerResponse.answer[1],
                round_q3_p1: answerResponse.answer[2]
              }, {
                where: {
                  round_id: answerResponse.round
                }
              }).then(
                resolve()
              ).catch(err =>
              ...
              models.broquiz_round.update({
                round_p2_score: score,
                round_q1_p2: answerResponse.answer[0],
                round_q2_p2: answerResponse.answer[1],
                round_q3_p2: answerResponse.answer[2]
              }, {
                where: {
                  round_id: answerResponse.round
                }
      ...
      Promise.all([updateRounds]).then(function() {
        var selectAllRounds_p1 = new Promise(function(resolve, reject) {
          models.broquiz_round.findAll({
            attributes: [
              [sequelize.fn('sum', sequelize.col('round_p1_score')), 'p1score']
            ],
            where: {
              round_game: answerResponse.game
            }
        ...
        var selectAllRounds_p2 = new Promise(function(resolve, reject) {
          models.broquiz_round.findAll({
            attributes: [
              [sequelize.fn('sum', sequelize.col('round_p2_score')), 'p2_score']
            ],
            where: {
              round_game: answerResponse.game
            }
        ...
        Promise.all([selectAllRounds_p1, selectAllRounds_p2]).then(function(gamepoints) {
          var updateGames = new Promise(function(resolve, reject) {
            ...
            models.broquiz_game.update({
              game_p1_points: gamepoints[0],
              game_p2_points: gamepoints[1]
            }, {
              where: {
                game_id: answerResponse.game
              }
            }).then(
              result => {
                console.log(result)
